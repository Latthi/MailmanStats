...
class T:
    ...
    def arcsub(self, x, y, radius, start, theta):
    ...
    def path_arc(self, x, y, radius, ratio, start, end):
        ...
        if ratio != 1.0:
            ...
            while deg < end:
                ...
                self.arcsub(x, y, radius, deg, theta/2)
                deg = deg + theta
	...
	if self.__lines == []:
        ...
        stream_obj_id = self.__define_stream_obj(fp, string.join(self.__lines))

        fontstr = ""
        for font_name, font_id in self.__registered_fonts.items():
            obj_id = self.__define_font_obj(fp, font_name, font_id)
            fontstr = fontstr + "/F%d %d 0 R " % (font_id, obj_id)
...
>> >>""" % (pages_obj_id, stream_obj_id, 
	self.__xmin-1, self.__ymin-1, self.__xmax+1, self.__ymax+1, fontstr))
    
        info_obj_id = self.__define_obj(fp, """<</Producer (PyChart)>>""")
